...
class Menu(tk.Frame): 
    ...
    def __init__(self, parent, controller):  
        ...
        button1 = ttk.Button(self, text ="Fertilidade\nRelativa", command = lambda : controller.show_frame(fertilidadeRelativa)) 

        button1.grid(row = 1, column = 1, padx = 10, pady = 10) 

class fertilidadeRelativa(tk.Frame): 
    
    def grafFertilidadeMulher(self, parent, controller):  
        ...
        canvas = FigureCanvasTkAgg(master =  __init__.container)
        canvas.draw()
        canvas.get_tk_widget().grid(row = 1, column = 1)

    def grafFertilidadeHomem(self, parent, controller):
        fig = Figure(figsize = (5, 5), dpi = 80)
        fig.clear()
        plotf = fig.add_subplot(111)
    ...
    def grafFertilidadeAmbos(self, parent, controller):
        fig = Figure(figsize = (5, 5), dpi = 80)
        fig.clear()
        plotf = fig.add_subplot(111)
    ...
    def __init__(self, parent, controller): 
        ...
        button1 = ttk.Button(self, text ="Mulher", 
                            command = lambda : fertilidadeRelativa.grafFertilidadeMulher) 
 
        button1.grid(row = 1, column = 1, padx = 10, pady = 10)

        button2 = ttk.Button(self, text ="Homem", 
                            command = lambda : fertilidadeRelativa.grafFertilidadeHomem) 

        button2.grid(row = 1, column = 2, padx = 10, pady = 10) 
 
        button3 = ttk.Button(self, text ="Ambos", 
                            command = lambda : fertilidadeRelativa.grafFertilidadeAmbos) 
